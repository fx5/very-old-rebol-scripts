...
flatten: func [blk [series!] /local tmp] [
	blk: copy blk
	while [not tail? blk] [
		either (type? first blk) = (type? blk) [
			tmp: first blk
			remove blk
			insert blk tmp
		] [
...
remove-texts: func [
	"Entfernt den Text aus einem HTML-Block (nicht Tree!)"
	blk [block!] /local old
] [
	old: blk
	forall blk [
		while [all [not empty? blk not tag? first blk]] [remove blk]
	]
	old
]

parse-html: function [str [string! file! url!]] [out allstr buf tmp] [
	if find reduce [file! url!] type? str [str: read str]
	h-data: [
		any [
			h-comment |
			copy data h-tag	 (append out data) |
			copy data h-text (append out data) |
			skip
		]
	]
	h-comment: ["<!--" thru "-->"]
	h-tag: ["<" some h-tag-data ">"]
	h-tag-data: [
		{"} thru {"}	|
		{'} thru {'}	|
		no-klammerzu
	]
	h-text: [some no-klammerauf]
	no-klammerzu: remove/part charset [#"^(0)" - #"^(ff)"] ">"
	no-klammerauf: remove/part charset [#"^(0)" - #"^(ff)"] "<"
	out: copy []
	parse str h-data
	forall out [
		either equal? first first out #"<" [
		  error? try [
			tmp: copy/part next first out back tail first out
			change out to-tag tmp
		  ]
		] [
			change out de-entity first out
		]
	]
	head out
]

count: function [str [string!] search [string!]] [count] [
	count: 0
	while [str: find/tail str search] [ count: count + 1]
	count
]
...
match-tag?: func [stag etag] [
	all [
		...
		equal?
			first parse stag ">"
			first parse next etag ">"
	]
]

html-tree: func [block /local pos tag pos2 mrk] [
	block: copy block
	pos: block
	while [not tail? pos] [
		if end-tag? tag: first pos [
			pos2: next pos
			while [ all [
				not head? pos
				not match-tag? first pos tag
			]] [
				pos: back pos
			]
			either match-tag? first pos tag [
				mrk: copy/part pos pos2
...
killtags: function [str] [html-code txt text] [
	text: copy ""
	html-code: [
		"<" tag-rest |
		"<" |
		copy txt [skip to "<" | skip to end] (append text txt)
	]
	tag-rest: [
		">" |
		{"} thru {"}  tag-rest |
	...
	parse str [some html-code]
	text
]



de-entity: function [str] [mrk str2] [
	str2: str: copy str
	while [str: find str "&"] [
		either all [ parse str [skip copy mrk to ";" to end] (length? mrk) < 10 ][
			remove/part str (length? mrk) + 2
			str: insert str conv-entity mrk
		] [str: next str]
	]
	str2
]

conv-entity: function [str] [mrk] [
	if (first str) = #"#" [error? try [return to-char to-integer next str]]
	error? try [ return to-char select entity-data str ]
	return #" "
]


main-part: func [block /local full main?] [
	full: length? get-flow-text html-to-text form block
	main?: func [block] [
		(length? get-flow-text html-to-text form block) > (full / 1.5)
	]
...
html-to-text: func [
	...
	html [string! block!] "Der HTML-Baum oder string"
	/local out
] [
	out: copy ""
	either string? html [
	...
	foreach entry html [
		...
		either string? entry [
			replace/all entry "^/" " "
			replace/all entry "^M" " "
			while [find entry "  "] [replace/all entry "  " " "]
			append out entry
		] [
			entry: switch first parse entry none [
				"br" [newline]
				"p"  ["^/^/"]
				"li"  ["^/* "]
				"tr"  ["^/^/"]
...
get-flow-text: func [
	"Holt fliesstext"
	str [string!] "Der Text (nicht HTML!)"
	/local out
] [
	out: copy ""
	str: parse/all str "^/"
	foreach entry str [
		if all [
			30 <= length? entry ; Mindestens 30 Zeichen
			5 <= length? parse/all entry " " ; Mindestens 5 Wörter
		] [
			append out rejoin [entry "^/"]
		]
	]
	if not tail? out [remove back tail out]
	out
]
...
<!ENTITY quot    CDATA "&#34;"   -- quotation mark = APL quote,
   ...
   <!ENTITY amp     CDATA "&#38;"   -- ampersand, U+0026 ISOnum -->
	...
	thru "CDATA"
	any " "
	{"&#} copy val to ";" (insert entity-data reduce [name to-integer val])
  ]	thru newline end ]
